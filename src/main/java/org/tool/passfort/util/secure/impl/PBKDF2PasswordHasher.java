...
public class PBKDF2PasswordHasher implements PasswordHasher {
    ...
    private static final int KEY_LENGTH = 256; // 哈希结果长度（256位）
    ...
    public byte[] hashPassword(String password) throws Exception {
        ...
        SecretKeyFactory factory = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA256");// 获取 SecretKeyFactory
        byte[] hash = factory.generateSecret(spec).getEncoded();// 生成哈希值
